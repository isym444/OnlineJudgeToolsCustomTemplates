...
/*
    idea for 2d prefix sum building is convert/shift to i+1,j+1 notation
    then accumulate rows (remember to go to boundary i.e. foi(0,N+1) when you are accumulating rows)
...
for (int i = 0; i < N; i++)
{
    for (int j = 0; j < N; j++)
    {
        imos[i + 1][j + 1] = (S[i][j] == '#') ? 1 : 0;
    }
}

// prefix sums: first vertical, then horizontal
for (int i = 0; i < N; i++)
{
    for (int j = 0; j <= N; j++)
    {
...
// f(p, a,b,c,d): count of cells in rectangle [a,b) x [c,d)
// if p==true -> count '#'
// if p==false -> count '.' (i.e., area - '#')
auto f = [&](bool p, int a, int b, int c, int d) -> int
{
    int x = 0;
    x += imos[a][c];
    x -= imos[b][c];
...
ll getSubgridSum3D(const vector<vector<vector<ll>>> &prefix, ll x1, ll y1, ll z1, ll x2, ll y2, ll z2)
{
    ...
    if (x1 > 0)
        sum -= prefix[x1 - 1][y2][z2];
    if (y1 > 0)
        sum -= prefix[x2][y1 - 1][z2];
    if (z1 > 0)
        sum -= prefix[x2][y2][z1 - 1];

    if (x1 > 0 && y1 > 0)
        sum += prefix[x1 - 1][y1 - 1][z2];
    if (x1 > 0 && z1 > 0)
        sum += prefix[x1 - 1][y2][z1 - 1];
    if (y1 > 0 && z1 > 0)
        sum += prefix[x2][y1 - 1][z1 - 1];

    if (x1 > 0 && y1 > 0 && z1 > 0)
        sum -= prefix[x1 - 1][y1 - 1][z1 - 1];
