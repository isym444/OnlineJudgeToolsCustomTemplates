...
#define fo(from_0_to_non_incl_to) \
  for (int i = 0; i < from_0_to_non_incl_to; i++)
// h CAREFUL if you put an expression as an argument it will give bugs, better
// assign expression to variable then put that in the foi() as argument
...
#define pb push_back
#ifdef isym444_LOCAL
#define NYAN_DEBUG freopen("/dev/tty", "w", stderr);
#else
#define NYAN_DEBUG
#endif
#define nyan                         \
...
// N.b. next() for next node in list, insert(iterator, value) "inserts" value at
// position right before iterator
typedef list<ll> dll;
...
// multiset provides automatic ordering on insertion but unlike set, keeps
// duplicate/multiple items of same value n.b. set also provides autoamtic
// ordering on insertion n.b. maps are also sorted automatically on insertion
// according to key order .count(x) O(num_of_x+logN) .find(x) O(logN) -> so use
// find over count if possible .insert(x) O(logN) -> inserts s.t. sorted order
// is maintained .erase(x) O(logN) begin() O(logN)
...
#define vecsum(vectorName) \
  accumulate((vectorName).begin(), (vectorName).end(), 0)
#define isvalid(checking, min_boundary, max_boundary) \
  (0 <= checking and checking < max_boundary)

/*/---------------------------Base Conversions----------------------/*/
// #define itobin(x) bitset<32> bin(x)
#define itobin(intToConvertTo32BitBinaryNum) \
  std::bitset<32>(intToConvertTo32BitBinaryNum)
#define bintoi(binaryNum32BitToConvertToInt) \
  binaryNum32BitToConvertToInt.to_ulong()
#define binstoi(binaryStringToConvertToInt) \
  stoi(binaryStringToConvertToInt, nullptr, 2)
#define binstoll(binaryStringToConvertToInt) \
  stoll(binaryStringToConvertToInt, nullptr, 2)

/*/---------------------------Bits----------------------/*/
#define setbits(decimalnumber) __builtin_popcountll(decimalnumber)

/*/---------------------------Strings----------------------/*/
#define stringSplice(str, i, j) \
  (str).erase(i, j) // j is the length of string to erase starting from index i
#define string_pop_back(str) (str).pop_back()
#define substring(str, i, j) \
  (str).substr(i, j) // j is the length of substring from i
...
inline OStream &operator<<(OStream &os, const pair<T, U> &pa)
{
...
/*/---------------------------Custom library - most used
 * only----------------------/*/

/*/---------------------------Syntax hints for mint once import
 * mint.cpp----------------------/*/
...
//  e.g. modint998244353 a = modint998244353(x); // `a` now represents `x`
//  modulo 998244353 using mint = modint998244353; Custom operator<< for
//  modint998244353 How to use the ACL modular exponentiation function? e.g. to
//  do pow(10,6) mint(10).pow(6)
...
#ifdef isym444_LOCAL
const string COLOR_RESET = "\033[0m", BRIGHT_GREEN = "\033[1;32m",
             BRIGHT_RED = "\033[1;31m", BRIGHT_CYAN = "\033[1;36m",
             NORMAL_CROSSED = "\033[0;9;37m", RED_BACKGROUND = "\033[1;41m",
             NORMAL_FAINT = "\033[0;2m";
#define dbg(x)                                                          \
  std::cerr << BRIGHT_CYAN << #x << COLOR_RESET << " = " << (x)         \
            << NORMAL_FAINT << " (L" << __LINE__ << ") " << COLOR_RESET \
            << std::endl
#define dbgif(cond, x)                                                         \
...
{
  static_assert(std::is_arithmetic<T>::value && std::is_arithmetic<U>::value,
                "floordiv requires arithmetic types");

  if constexpr (std::is_floating_point<T>::value ||
                std::is_floating_point<U>::value)
  {
    // Handle case where either n or d is a floating-point number
    // Perform the division as floating-point operation and use std::floor to
    // round down
    return static_cast<decltype(n / d + 0)>(
        std::floor(n / static_cast<double>(d)));
  }
  else
  {
    // Handle case where both n and d are integers
...
// index of the first occurrence of x. If x is not present in the vector, it
// returns the index where x can be inserted while keeping the vector sorted
template <class T>
int indlb(const std::vector<T> &v, const T &x)
{
  return std::distance(v.begin(), std::lower_bound(v.begin(), v.end(), x));
}
// index immediately after the last occurrence of x. If x is not present, like
// the lower bound, it returns the index where x can be inserted to maintain
// order
template <class T>
int indub(const std::vector<T> &v, const T &x)
{
